			...
			if (socketARevisar) {								//Reviso actividad en cpus
				...
				//todo eliminar pcb si no termino bien el quantum
					...
					if (FD_ISSET(nucleo_servidor, &descriptores)) { 			//aceptar cliente
						...
						nuevo_cliente = eperarConexion(nucleo_servidor,autentificacion);
						if (nuevo_cliente == -1) {
...
PCB* crearPCB(char* codigo) {
	...
	pcb->id = (el sockets de la consola a la que pertenese el codigo);
	pcb->indices = metadata;
	pcb->paginas_codigo = strlen(codigo)/tamanioPaginaUMC;
